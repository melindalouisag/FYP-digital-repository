...
export const adminApi = {
  ...
  students(): Promise<AdminStudentTrackingGroup[]> {
    return getJson('/api/admin/students');
  },

  studentDetail(studentUserId: number, caseId?: number): Promise<AdminStudentTrackingDetail> {
    const query = caseId ? `?caseId=${caseId}` : '';
    return getJson(`/api/admin/students/${studentUserId}${query}`);
  },
